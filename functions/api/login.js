...
function cookieHeader(setCookies) {
  ...
  for (const raw of setCookies || []) {
    ...
    if (eq > 0) jar.set(pair.slice(0, eq), pair.slice(0, eq) + "=" + pair.slice(eq + 1));
  }
...
export async function onRequestPost(context) {
  try {
    ...
    if (!configured.SECRET_PIN) return json({ success: false, error: "Administrative PIN is not configured." }, 500);
    if (pin !== env.SECRET_PIN) return json({ success: false, error: "Invalid administrative passcode." }, 401);
    ...
    const allCookies = [...pageCookies, ...loginCookies];
    ...
    if (!location && login.status >= 200 && login.status < 300) {
      ...
      dashboardResponse = /logout/i.test(loginBody) && !/name=["']login_userid["']/i.test(loginBody);
    }
    ...
    if (!sessionSuccess) {
      ...
      return json({ success: false, error: "Koha rejected the staff login.", status: login.status, has_session_cookie: hasSessionCookie(allCookies), redirect: Boolean(redirectSuccess) }, 502);
    }

    // The membership application only needs successful authentication here.
    // The browser is redirected to its own index.html by login.html.
    return json({ success: true, redirect: "/index.html" });
  } catch (error) {
